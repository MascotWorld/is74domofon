...
def _strip_optional_quotes(value: str) -> str:
    """Strip matching single or double quotes from dotenv values."""
...
def _load_dotenv_overrides() -> dict[str, str]:
    """Load optional overrides from a local .env file."""
    values: dict[str, str] = {}
    ...
    for env_path in _iter_env_paths():
        ...
        try:
            ...
            _LOGGER.warning("Failed to read %s: %s", env_path, err)

    return values


def _get_setting(name: str, *, required: bool = True, default: str | None = None) -> str:
    """Read a setting from process env or a local .env file."""
    value = os.getenv(name) or _load_dotenv_overrides().get(name) or default
    if required and not value:
        raise RuntimeError(
            f"Required setting {name} is not configured. Create .env from .env.example."
        )
    return value or ""


@lru_cache(maxsize=1)
def _get_runtime_settings() -> dict[str, str]:
    """Return runtime settings sourced from .env."""
    return {
        "user_agent": _get_setting("IS74_USER_AGENT"),
        "fcm_project_name": _get_setting("IS74_FCM_PROJECT_NAME"),
        "fcm_app_id": _get_setting("IS74_FCM_APP_ID"),
        "fcm_api_key": _get_setting("IS74_FCM_API_KEY"),
        "fcm_project_number": _get_setting("IS74_FCM_PROJECT_NUMBER"),
        "device_model": _get_setting("IS74_DEVICE_MODEL", default="Android Device"),
    }
...
async def initialize_fcm() -> str:
    ...
    fcm_config = FcmRegisterConfig(
        settings["fcm_project_name"],
        settings["fcm_app_id"],
        settings["fcm_api_key"],
        settings["fcm_project_number"],
    )
...
async def _crm_register_device(
    ...
    body = {
        ...
        "deviceId": device_id,
        "deviceName": settings["device_model"],
        "platform": "google",
        "pushToken": fcm_token,
        "sendingPush": True,
    }
